...
public class ProductServlet extends HttpServlet {
	...
	private void handleProductList(HttpServletRequest req, HttpServletResponse resp)
		...
		// Calculate discounted prices for all products in the list
		if (products != null) {
			for (Product p : products) {
				try {
					BigDecimal discountedPrice = promotionService.getDiscountedPrice(p.getMaSP());
					p.setDiscountedPrice(discountedPrice);
				} catch (Exception e) {
					logger.warn("Failed to get discounted price for product {}: {}", p.getMaSP(), e.getMessage());
				}
			}
		}
		...
		try {
			...
			if (p != null) {
				...
				if (cat != null) {
					...
					promotionService.populateDiscountedPrices(relatedProducts);
